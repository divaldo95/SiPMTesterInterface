...
function ModeSelectButtonGroup(props) {
    ...
    const { measurementData, updateSiPM, areAllPropertiesSet, isAnyMeasurementRunning } = useContext(MeasurementContext);
    ...
    return (
        <>
            <div className="btn-group" role="group" aria-label="sensor block with settings">
                <button disabled={isMeasurementRunning()} onClick={(e) => { e.preventDefault(); handleClick("IV"); }} className={`btn ${isAllSets().allIVSet ? 'btn-success' : 'btn-secondary'}`}>
                    IV <i className={`ms-1 bi ${isAllSets().allIVSet ? 'bi-dash-square' : 'bi-check-square'}`}></i>
                </button>
                <button disabled={isMeasurementRunning()} onClick={(e) => { e.preventDefault(); handleClick("SPS"); }} className={`btn ${isAllSets().allSPSSet ? 'btn-success' : 'btn-secondary'}`}>
                    SPS <i className={`ms-1 bi ${isAllSets().allSPSSet ? 'bi-dash-square' : 'bi-check-square'}`}></i>
                </button>
                <button disabled={isMeasurementRunning()} onClick={(e) => { e.preventDefault(); handleClick("Both"); }} className={`btn ${isAllSets().allIVSet && isAllSets().allSPSSet ? 'btn-success' : 'btn-secondary'}`}>
